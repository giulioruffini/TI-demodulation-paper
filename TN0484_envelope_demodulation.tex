...
\emph{Parity argument.} Index Fourier components by the carrier harmonic $k$ (frequency
near $k\omega_c$). The field forces only at \emph{odd} $k=\pm1$; the linear operator
$(a+i\omega_0)$ preserves $k$; and the cubic $|z|^2z=z\bar z z$ maps
$\text{odd}\times\text{odd}\times\text{odd}\to\text{odd}$. Hence $z$ has support only at
odd $k$ to all orders in $\varepsilon$, and its weight at the beat ($k=0$, even) is
identically zero. Equivalently, demodulation is the $O(\varepsilon^2)$ \emph{square-law}
term $\half\Sig''\varepsilon^2 m$ (Eq.~\eqref{eq:demod}); the SL normal form has \emph{no}
even-order term, so that contribution vanishes. \textbf{The SL resonator amplifies
...
demodulate. Code: \texttt{sl\_dissociation.py} (self-contained).}
\label{fig:sl}
\end{figure}

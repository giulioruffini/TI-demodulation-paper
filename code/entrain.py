...
# (a) tongue grid
dfg=np.linspace(f0-4.0,f0+4.0,48); epsg=np.linspace(0.0,1.3,16)
...
def integ_vec(p,eps,Om,t_set,t_meas,dt,dec=5):
    ...
    return fdom, vmax-vmin, lockin
fdom,amp,li_grid=integ_vec(p,eps,Om,5.0,5.0,dt)
detune=np.abs(fdom-DF.ravel())
locked=(detune<0.25).astype(float).reshape(DF.shape)      # 1:1 lock (binary boundary)
np.savez(os.path.join(HERE,"entrain.npz"),dfg=dfg,epsg=epsg,locked=locked,
         lockin=li_grid.reshape(DF.shape),detune=detune.reshape(DF.shape),
         f0=f0,amp0=amp0,p0=p0)
print("tongue done; locked fraction:", locked.mean())

# (b) one drive amplitude: lock-in + amplitude + dominant freq vs Delta f (finer, longer)
eps_b=0.6; dfb=np.linspace(f0-4.0,f0+4.0,61); Omb=2*np.pi*dfb
li,ampb,fd=integ(np.full(len(dfb),p0),eps_b,Omb,6.0,8.0,dt)
np.savez(os.path.join(HERE,"entrain_b.npz"),dfb=dfb,li=li,ampb=ampb,fd=fd,
         f0=f0,amp0=amp0,eps_b=eps_b)
print("panel b done")

...
explicitly \emph{network} mechanism at the mesoscopic scale whose two ingredients
factorize. The population sigmoid \emph{detects}: in a neural mass (a population of
$\sim$$10^4$ neurons) it is a square-law demodulator,
$A_\Omega\simeq\half\Sig''(v^*)\,\varepsilon^2 m$ (recovered envelope amplitude
$A_\Omega$; sigmoid curvature $\Sig''$ at operating point $v^*$; post-coupling
polarization $\varepsilon$; modulation depth $m$), a curvature inherited from the
single neuron's input--output nonlinearity and requiring no specific channel. The
network \emph{amplifies}: a second-order-synapse loop near a supercritical Hopf
bifurcation resonantly boosts the recovered envelope at its natural frequency, the
gain growing as $1/\gamma$ as the damping $\gamma$---the distance to the Hopf---vanishes.
The two are separable: holding detection fixed, this gain is set by the network's
proximity to criticality and is tuned by the coupling itself, and---being sharply
frequency-selective---it amplifies the envelope-frequency response (population
\emph{timing}) far more than the mean firing rate. In a Jansen--Rit column an
alpha-free carrier input yields a clean alpha output obeying the square law,
carrier-independent at fixed polarization (the applied-field threshold rising with
carrier frequency $f_c$) and sign-reversed by the curvature across the sigmoid
inflection. The two-band laminar model (LaNMM) and the exact next-generation mean
field (NMM2) confirm it---the latter with the demodulating nonlinearity \emph{and}
the amplifying coupling both derived rather than postulated. The mechanism is
state-dependent through the operating point (PEIX) and connects TI to
cross-frequency coupling and hierarchical amplitude modulation. The message:
\emph{wherever a fast-enough nonlinearity samples the carrier, the population sigmoid
demodulates and the network amplifies}---the cortical column acting as a
tuned AM receiver amplifier.
...
stochastic resonance, whose inverted-U signature tES and transcranial random-noise stimulation (tRNS) display
...
attention, arousal and pathology (PEIX), so TI and transcranial magnetic stimulation
(TMS) efficacy should vary with brain

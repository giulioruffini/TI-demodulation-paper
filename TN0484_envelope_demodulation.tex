...
modulation is subtle, but two factors make it count: the envelope sits in the band
the membrane passes, and the population near a Hopf amplifies it by $G\!\sim\!1/\gamma$
(Eq.~\eqref{eq:gain}). Subtle demodulation at fast elements, resonantly amplified at
the population, is thus a plausible route for TI at realistic carriers---consistent
...
Because tES---and TI, a tES with a kHz carrier---is weak and \emph{modulatory}, the
...
drive, $V(t)=V_0+\delta V_{\mathrm{tES}}(t)+\eta(t)$, there is no strict minimum
field: near threshold a tiny polarization biases spike probability and timing through
stochastic resonance, whose inverted-U signature tES and tRNS display
\cite{reato2010,terney2008,vandergroen2016,liu2018}. This maps onto the neural mass,
whose sigmoid \eqref{eq:sigmoid} is itself a mean-field stochastic-resonance
device---its smooth slope arises from the spread of thresholds and noise across the
population---so a weak bias shifts the mean rate through $\Sig$ (gain $\Sig'$) while
the demodulated envelope emerges through the curvature $\Sig''$.
...
$A_{\rm open}$ are constant. \emph{(a)} The demodulated $A_\Omega$ diverges as the
coupling $C\,(=J)$ approaches the Hopf $C^*$. \emph{(b)} The envelope resonance
...
\emph{(a)} The peak demodulated response of $r_E$ diverges as $J\!\to\!J^*$ (gamma
Hopf, $\bar\eta=0$). \emph{(b)} The gain grows as $1/\gamma$ (dashed), saturating near
...
DC shift of the mean pyramidal firing rate stays flat at a few mHz, and even changes
sign, throughout (Fig.~\ref{fig:timing}). Near criticality the recovered envelope
...
acting (code: \texttt{timing\_not\_rate.py}, \texttt{make\_timing\_fig.py}).
...
driven toward the Hopf through the coupling at fixed $\Sig''(v^*)$, \emph{(a)} the AC
...
but barely amplified. The amplification is therefore not only state- but \emph{coupling}-tunable: holding detection fixed, the demodulated gain grows toward $1/\gamma$ as the network is driven to criticality by its own connectivity (\S\ref{sec:jcurve}), in the heuristic JR column and, with the inter-neuron coupling and the rectifier both derived, in the exact NMM2 mean field. Both halves are thus strongly \textbf{state-dependent}---detection

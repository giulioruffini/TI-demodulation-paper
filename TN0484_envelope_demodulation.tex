...
\emph{Entrainment above the Hopf} (Fig.~\ref{fig:entrain}) requires a different read-out:
...
(Fig.~\ref{fig:entrain}a) is the locked set over the $(\Delta f,\varepsilon)$ grid at
fixed $p$; the locking plateau (Fig.~\ref{fig:entrain}b) is $f_{\rm out}$ vs $\Delta f$ at
...
$\max-\min$ of $v$. Two subtleties matter. (i)~The $\Delta f$ window is finite
($f_0\pm5$~Hz), so the locking ranges \emph{nearest} the Hopf---where the tongue is
widest---are lower bounds (grid-clipped), and a fixed tolerance/grid sets the absolute
scale, not the trend. (ii)~As onset is approached the cycle amplitude $\to0$ and the
...
The same holds at the spiking level. Integrating the underlying QIF network---$N=2000$
...
NMM2 mean field, and the mean rate stays flat ($\sim$1.1$\times$) while the
envelope-locked timing grows $14$--$22\times$ (Fig.~\ref{fig:qif_timing}); the full
microscale analysis is in App.~\ref{app:qif}.
...
generator): TI realigns spike timing, not mean rate.} $N=2000$ excitatory and inhibitory
...
2000 shown), blue the population rate $r_E(t)$, red the TI envelope; the boxed value is
...
gains a $\Delta f$-locked component ($\sim$22$\times$) while the intrinsic $\sim$54~Hz
...
$r_E(t)$: the spiking network ($N=2000$) matches the exact NMM2 mean field in the gamma
band and in mean rate $\langle r_E\rangle$. \emph{(b)} Fold change of the mean rate
$\langle r_E\rangle$ and of the envelope-locked timing (lock-in of $r_E$ at $\Delta f$)
from TI off to on: the mean rate is flat ($\sim$1.1$\times$) while the timing grows
$14$--$22\times$, in both the forced and oscillatory regimes.}
...
underlying spiking network directly---$N=2000$ QIF neurons per population, parameters
...
coincidence. Timing is quantified by the quadrature
lock-in of $r_E(t)$ at $\Delta f$ over the measurement window.
...
regimes the mean rate is essentially unchanged ($\sim$1.1$\times$) while the
envelope-locked component of the population rate grows $14$--$22\times$

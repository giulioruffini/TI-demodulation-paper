...
pg=np.array([150,185,215,245,268,285,297,305,310,313],float)
...
# locking range at fixed eps: Delta f grid relative to each f0. Wide window (+-12 Hz)
# so near-Hopf locking ranges are NOT grid-clipped; fine spacing for a clean width.
nd=73; half=12.0; rel=np.linspace(-half,half,nd); dlo=2*half/(nd-1)
P=[];DF=[]
for i,p in enumerate(pg):
    P+=[p]*nd; DF+=list(f0[i]+rel)
P=np.array(P);DF=np.array(DF);Om=2*np.pi*DF
fdom,_=run(P,eps,Om,5.0,6.0,dt)
locked=(np.abs(fdom-DF)<0.30).reshape(len(pg),nd)
lock_range=locked.sum(1)*dlo   # Hz
clipped=locked[:,0]|locked[:,-1]   # True if the locked band reaches a window edge
print(" p     dist=pH-p  f0     cycAmp   lockRange(Hz)  clipped")
for i in range(len(pg)):
    print(f"{pg[i]:5.0f}  {pH-pg[i]:7.0f}  {f0[i]:5.2f}  {amp0[i]:6.2f}  {lock_range[i]:6.2f}   {bool(clipped[i])}")
np.savez(os.path.join(HERE,"entrain_crit.npz"),pg=pg,pH=pH,f0=f0,amp0=amp0,
         lock_range=lock_range,clipped=clipped,eps=eps,half=half)
print("saved")

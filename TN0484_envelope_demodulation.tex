...
amplifies oscillatory timing far more than mean rate---entrains spike timing without
...
manufactured by the network, which adds only the resonant \emph{amplification}
(\S\ref{sec:carrier}). The nonlinearity is cellular in origin; our claim is only that
its demodulation is read out at the population level by the generic sigmoid, without
...
Equation~\eqref{eq:demod} contains no $\omega_c$: at fixed post-coupling polarization
$\varepsilon$ the demodulated amplitude is independent of the carrier (as a function of
the applied field it rolls off with $f_c$, \S\ref{sec:khz}). Under the direct coupling \eqref{eq:coupling} the carrier
...
only as $f_c$ approaches the natural band. As a function of
...
the nonlinearity sits at a fast element ($\tau\!\approx\!0.2$~ms): the closed-loop
$A_\Omega$ is carrier-independent for direct coupling, follows the fast element into
the kHz, and collapses as $1/f_c^2$ for the slow soma ($\tau_m\!\approx\!16$~ms),
exactly as Eq.~\eqref{eq:khz} predicts. A $2$~kHz carrier through the fast element
...
drive instead phase-locks that limit cycle within an Arnold tongue---the entrainment
regime of Fig.~\ref{fig:res} and the LaNMM and NMM2 maps---where the response is set by
the cycle amplitude and the locking range, not by a diverging $1/\gamma$ gain.
Quantifying that regime therefore calls for different metrics---entrainment
(Arnold-tongue width, phase-locking) and modulation of the oscillation amplitude,
rather than the forced-response lock-in used here, the two coinciding only at onset.
Because the demodulated drive is weak, on the autonomous side it biases phase (timing)
more than amplitude, consistent with the timing-not-rate signature
(\S\ref{sec:timing}). We show this regime directly above the Hopf in
Fig.~\ref{fig:entrain}: a direct drive into the autonomous JR alpha cycle locks it
within an Arnold tongue ($1{:}1$ frequency capture over a band that widens with drive),
phase entrainment quantified by the locking range rather than a $1/\gamma$ amplitude gain.
...
entrains population \emph{timing} at $\Delta f$ while leaving the mean \emph{rate}
...
acting (code: \texttt{timing\_not\_rate.py}, \texttt{make\_timing\_fig.py}). The same
...
$\sim$1. The envelope entrains timing, not rate---the population analog of the
...
\caption{\textbf{QIF spiking network (microscale of NMM2 PING): TI realigns spike
timing, not mean rate.} $N=2000$ excitatory and inhibitory QIF neurons; black ticks are
$E$ spikes (120 neurons shown), blue is the population rate $r_E(t)$, red is the TI
envelope. \emph{(a)} Forced ($\bar\eta<\bar\eta_{\rm Hopf}$), TI off: asynchronous
firing, flat rate. \emph{(b)} Forced, TI on ($\Delta f=55$~Hz): spikes bunch into the
envelope. \emph{(c)} Entrained ($\bar\eta>\bar\eta_{\rm Hopf}$), TI off: free-running
gamma, no $\Delta f$ lock-in. \emph{(d)} Entrained, TI on ($\Delta f=42$~Hz): gamma
bursts gate to the TI beat---the population rate gains a $\Delta f$-locked component
($\sim$22$\times$), while the intrinsic gamma persists. Boxed: lock-in at $\Delta f$,
off$\to$on.}
...
it---identifies where the carrier-sampling nonlinearity may live. The TI literature
...
(entrained) the free-running gamma is \emph{gated} by the TI beat: its bursts group
into the detuned $\Delta f=42$~Hz envelope---adding a $\Delta f$-locked component to
the population rate---while the intrinsic $\sim$54~Hz gamma persists
...
$14$--$22\times$, in both the forced and entrained regimes.}
...
diverging $1/\gamma$ gain of the stable-focus side (Fig.~\ref{fig:jcurve}). Like that gain, entrainment is enhanced near criticality---the tongue widens as the Hopf is approached and the autonomous cycle weakens---so the column is most responsive near the critical point, to forcing below the Hopf and to entrainment above. Code:

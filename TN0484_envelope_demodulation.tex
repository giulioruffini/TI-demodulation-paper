...
\textbf{Author:} & Giulio Ruffini, Borja Mercadal, Raul de Palma, Alex Just, \\ 
...
(autonomous $f_0\!\approx\!54$~Hz; these finite-$N$ values sit just above the exact
...
$\Delta f=55$~Hz \emph{at} the forced gamma resonance ($\bar\eta=0.85$, below the Hopf) and
$\Delta f=42$~Hz in the oscillatory regime ($\bar\eta=1.5$, above the Hopf)---a detuning of
$\approx$12~Hz below the autonomous $f_0\!\approx\!54$~Hz that places the beat outside the
...
(PAC), not frequency entrainment, since the $\approx$12~Hz detuning lies outside the
$1{:}1$ tongue: its bursts group into the $\Delta f=42$~Hz envelope while the intrinsic
$\sim$54~Hz gamma persists (Fig.~\ref{fig:qif_raster}c,d). The spiking network reproduces
...
from near zero to $0.19$--$0.76$ (Fig.~\ref{fig:qif_timing}b): TI realigns timing, not
...
8000, a representative sample), blue the population rate $r_E(t)$, red the TI envelope; the boxed value is
...
$\Delta f$-locked component as a real driven effect rather than coincidence. \emph{Insets:} \emph{(b)} the population rate folded on the $\Delta f$ beat phase (the
forced response \emph{is} at $\Delta f$); \emph{(d)} the gamma-band \emph{amplitude}
folded on the $\Delta f$ phase---taking the amplitude removes the gamma carrier, so the
near-commensurate $54/42$~Hz gamma cannot contaminate the fold (a raw-rate fold would).
Both are flat with TI off (gray) and show a clear $\Delta f$-locked modulation with TI
on (blue), making the envelope-locked timing direct where the raster alone leaves it
subtle.}
